...
def extract_resource
  ...
  @cmd = "/updates/parted" if @cmd.nil? or @cmd.length() == 0 
  Chef::Log.info("parted command to use: #{@cmd}")
end

def load_current_resource
  dev_name = @new_resource.name
  extract_resource
  @current = Chef::Resource::HadoopDisk.new(dev_name)  
...
=end
  pipe= IO.popen("#{@cmd} -m -s #{dev_name} unit cyl print") # this can return 1, but it's ok (if no partition table present, we'll create it)
...
=end
  ...
  if !recreate
    ...
    cmd = "#{@cmd} -s -m #{dev_name} mktable bsd"
    ...
    req.each { | params |  
      ...
      s = "#{@cmd} -m -s #{dev_name} unit cyl "
      s << "mkpart #{cur_cyl.to_i} #{(cur_cyl += cyls).to_i} " # #{params[:type]}    
      log("creating new partition #{idx+1} with:" + s)
